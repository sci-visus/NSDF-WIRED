...
We created 4 directories for each forecast ID that UBC provides:
```
.
├── BSC00CA12-01
├── BSC06CA12-01
...
2. In a for loop, we use `pandas` to create a list of every date from the start date and end date of the current iteration. We will loop through these dates next.
3. For each `date` in the list, we create the `url` to download the file.
4. Set the file name and directory to download the files. Each file is stored in a directory that corresponds to the forecast ID name.
...
This approach yielded the results we expected, all files downloaded were NetCDF files. Having all the files on our machine, we proceed to compiling the single data set.